 ...
 *
 * @namespace quotes_for_woocommerce
 * @since 2.3
 */
		...
		$button.on( 'click.notice-dismiss', function( event ) {
			...
			$this.fadeTo( 100 , 0, function() {
				...
				jQuery(this).slideUp( 100, function() {
					jQuery.post(
						qwc_dismiss_notice_params.admin_url,
						{
							action: qwc_dismiss_notice_params.prefix_of_plugin + "_tracker_dismiss_notice",
							tracking_notice : qwc_dismiss_notice_params.tracking_notice,
							security: qwc_dismiss_notice_params.tracking_notice
						},
						function( response ) {
							if ( 'success' === response ) {
								jQuery(this).remove();				
